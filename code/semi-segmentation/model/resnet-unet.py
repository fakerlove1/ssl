...
class Resnet_Unet(nn.Module):
    ...
    def __init__(self, in_channel, num_classes, backbone="resnet34", BN_enable=True, resnet_pretrain=False):
        super().__init__()
        self.BN_enable = BN_enable
        # encoder部分
        ...
        self.firstconv = nn.Conv2d(in_channels=in_channel, out_channels=64, kernel_size=7, stride=2, padding=3,
                                   bias=False)
...
if __name__ == '__main__':
    ...
    model = Resnet_Unet(in_channel=1, num_classes=4)
    y = model(x)
    print(y.shape)
