...
class ACDC(Dataset):
    def __init__(self, root=r"E:\note\ssl\data\ACDC", mode="train", transform=None):
        ...
        if self.mode == 'train':
            with open(self.root + "/trainval.txt", "r") as f1:
    ...
    def __getitem__(self, idx):
        ...
        image = np.load(os.path.join(self.root, "image", case))
        mask = np.load(os.path.join(self.root, "mask", case))
...
def get_loader(root=r'/kaggle/input/acdc-2/My-ACDC-2', batch_size=4, crop_size=(256, 256), label=0.2):
    """
    ...
    train_unlabel_dataloader = DataLoader(train_unlabel, batch_size=batch_size*2, num_workers=4, shuffle=True,)
